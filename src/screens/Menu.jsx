        ...
        {editOpen && (
          <div className="hero-editor">
            <div className="he-field">
              <span className="he-lab">한 달 목표 벌이</span>
              <div className="he-step">
                <button onClick={() => setMonthlyGoal((g) => g - 100000)} aria-label="목표 감소"><Icon name="minus" size={15} stroke={2.4} /></button>
                <span className="num">{manwon(monthlyGoal)}</span>
                <button onClick={() => setMonthlyGoal((g) => g + 100000)} aria-label="목표 증가"><Icon name="plus" size={15} stroke={2.4} /></button>
              </div>
            </div>
            <div className="he-field">
              <span className="he-lab">한 달 고정비</span>
              <div className="he-step">
                <button onClick={() => setMonthlyFixed((v) => v - 100000)} aria-label="고정비 감소"><Icon name="minus" size={15} stroke={2.4} /></button>
                <span className="num">{manwon(monthlyFixed)}</span>
                <button onClick={() => setMonthlyFixed((v) => v + 100000)} aria-label="고정비 증가"><Icon name="plus" size={15} stroke={2.4} /></button>
              </div>
            </div>
            <div className="he-field">
              <span className="he-lab">한 달 영업일</span>
              <div className="he-step">
                <button onClick={() => setWorkDays((d) => d - 1)} aria-label="영업일 감소"><Icon name="minus" size={15} stroke={2.4} /></button>
                <span className="num">{workDays}일</span>
                <button onClick={() => setWorkDays((d) => d + 1)} aria-label="영업일 증가"><Icon name="plus" size={15} stroke={2.4} /></button>
              </div>
            </div>
            <p className="fx-hint">월세·인건비 같은 한 달 값만 넣으면 영업일로 나눠 하루치를 계산해요 · 하루 고정비 {won(dailyFixed)}원</p>
          </div>
        )}
      </div>
